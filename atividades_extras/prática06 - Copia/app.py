...
@app.route('/')
def index():
    # Se ainda não tiver o arquivo cria ele vazio
    if not os.path.exists("arquivo.json"):
            with open('arquivo.json', 'w') as f:
                json.dump({}, f, indent=4)
    # Se ainda não tiver o arquivo cria ele vazio
    if not os.path.exists("compras.json"):
...
def login():
    if request.method == 'POST':
        ...
        if nome in session['usuarios'] and check_password_hash(session['usuarios'][nome]['senha'], senha):
            ...
            login_user(user)
            # Pega tudo que está no arquivo e guarda dentro da variável dados
            with open('arquivo.json', 'r') as f:
                dados = json.load(f)
            dados[current_user.id]={}
            # Pega tudo que está na variável dados e coloca dentro do arquivo
            with open('arquivo.json', 'w') as f:
                json.dump(dados, f)
